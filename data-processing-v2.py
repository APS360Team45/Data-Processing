...
def load_data(folder): 
  ...
  for fruit in os.listdir(folder):
    ...
    for ripeness, file in enumerate(os.listdir(n_path)):
        ...
        val_list = img_list[:100]
        train_list = img_list[100:]

        original_size = len(train_list)

        if original_size > 800:
          train_list = train_list[:800]
        while len(train_list) < 800:
          mod = random.randint(0, 5)
          train_list.append(mod)

        print(fruit, ripeness)
        print(len(train_list))
        print(len(val_list))

        for count, img in enumerate(val_list):
            ...
            val.append((transform(hsv_image), torch.tensor(label)))

        for count, img in enumerate(train_list):
            
            if count >= original_size:

              temp = random.randint(0, original_size-1)
              temp_str = train_list[temp]
            ...
            train.append((transform(hsv_image), torch.tensor(label)))
...
torch.save(train, f'train_dataset_v2.pth')
torch.save(val, f'val_dataset_v2.pth')
